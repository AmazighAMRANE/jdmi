...
def dechiffrer_message(fichier, dictionnaire):
    ...
    for pair in paires_chiffrees:
        if len(pair) == 2 and pair in dictionnaire:
            message_dechiffre += dictionnaire[pair] + " " # Remplacer "pair" par sa valeur dans la variable dictionnaire
        else:
...
fichier_dictionnaire = 'dictionnaire.txt' # Mettre le fichier contenant le dictionnaire
fichier_message = 'message.txt'	# Mettre le chemin vers le fichier contenant le message chiffré

...
def hangman(word):
    ...
    while attempts_left > 0 and not word_set.issubset(guessed_letters):
        ...
        user_input = input("Entrez une lettre que vous pensez être dans le mot: ").upper()
    ...
    if word_set.issubset(guessed_letters):
        ...
        webbrowser.open('https://www.youtube.com/watch?v=vcZRDvh3ddE') # Lecture du son en cas d'échec
        return False

def hash_verification():
    words = ["QUOICOUBEH","BANGER","LAURINE"]
    print('Avant d\'essayer de trouver l\'événement, vous devez prouver votre compréhension de la fonction de hachage.')
...
def mainHash():
    ...
    print("Vous devez entrer les ingrédients/indices que vous avez trouvés pour débloquer la prochaine épreuve.\n")
    print("Il y avait 7 ingrédients/indices à trouver.\n")

    found_ingredients = set()

    while len(found_ingredients) < 7:
        ingredient = input("Entrez un ingrédient/indice trouvé : ").strip().upper()
        print()
        if ingredient in correct_ingredients and ingredient not in found_ingredients:
            found_ingredients.add(ingredient)
            print("Ingrédients/indices trouvés jusqu'à présent :")
            for ing in correct_ingredients:
                if ing in found_ingredients:
                    print(f"- {ing}")
            print()
        else:
            print("Ingrédient/indice incorrect ou déjà trouvé, veuillez réessayer.\n")
    ...
    print("On cherche une abréviation de ce mot. Cette abréviation a la même valeur de hash que 'LAURINE'.")
    
    if hangman("BARBEC"):
        print("Correct ! Vous avez deviné le mot.")
        webbrowser.open('https://www.youtube.com/watch?v=0mfJn604GT4')
    else:
